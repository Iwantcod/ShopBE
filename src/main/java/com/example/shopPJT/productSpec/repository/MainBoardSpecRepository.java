...
public interface MainBoardSpecRepository extends JpaRepository<MainBoardSpec, Long> {
    @Query("SELECT new com.example.shopPJT.productSpec.dto.ModelNameDto(t.id, t.modelName, t.manufacturer)" + "FROM MainBoardSpec t WHERE t.modelName LIKE %:keyword%") // 키워드를 이용하여 조회
    Slice<ModelNameDto> searchByKeyword(Pageable pageable, @Param("keyword") String keyword);

    @Query("SELECT new com.example.shopPJT.productSpec.dto.ModelNameDto(t.id, t.modelName, t.manufacturer)" + "FROM MainBoardSpec t")
    Slice<ModelNameDto> findAllByCreatedAtDesc(Pageable pageable);

    @Modifying(clearAutomatically = true, flushAutomatically = true)
    @Query(value = """
        UPDATE main_board_spec AS s
        JOIN (
          SELECT
              spec_id,
              FLOOR(
          ...
          ON s.mainboardspec_id = a.spec_id
